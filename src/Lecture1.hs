...
subString start end str
  ...
  | otherwise =
      let resSize = (end - start) + 1
       in take resSize (drop start str)
...
strSum :: [Char] -> Int
strSum str = sumNums 0 (words str)
  where
    sumNums :: Int -> [[Char]] -> Int
    sumNums result nums =
      if null nums
        then result
        else sumNums (result + (read (head nums) :: Int)) (tail nums)
...
lowerAndGreater n list =
  ...
  where
    go :: Int -> Int -> [Int] -> [Char]
    go lowerCount greaterCount sortedList
      | null sortedList = show n ++ " is greater than " ++ show greaterCount ++ " elements and lower than " ++ show lowerCount ++ " elements"
      | n < head sortedList = go (lowerCount + 1) greaterCount (tail sortedList)
      | n > head sortedList = go lowerCount (greaterCount + 1) (tail sortedList)
      | otherwise = go lowerCount greaterCount (tail sortedList)
